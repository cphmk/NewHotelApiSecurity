...
public class SecurityController implements ISecurityController {
    ...
    public static SecurityController getInstance() { // Singleton because we don't want multiple instances of the same class
        ...
        securityDAO = new SecurityDAO(HibernateConfig.getEntityManagerFactory("hotelapisecurity"));
        return instance;
    }
    ...
    public @NotNull Handler addRole() {
        return (ctx) -> {
            ...
            try {
                ...
                UserDTO newUser = new UserDTO(updatedUser.getUsername(), updatedUser.getRoles().stream().map(r -> r.getRoleName()).collect(Collectors.toSet()));
                ...
                ctx.status(200).json(returnObject.put("msg", "Role " + newRole + " added to user").put("token", token).put("username", newUser.getUsername()));
